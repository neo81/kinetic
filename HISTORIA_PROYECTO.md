...
- Idioma y formatos:
  ...
  - La sesion activa ya localiza el detalle, seleccion de dias, temporizadores, series planificadas y registradas, omisiones, confirmaciones y finalizacion sin alterar la logica de CORE ni los datos de entrenamiento.
  ...
  - Novedades incorpora la version `2026.08.22`, con seis anuncios bilingues sobre navegacion movil, sesiones activas, descansos personalizados, idioma, Motor y experiencia PWA. La misma publicacion se conserva en Supabase y en el respaldo local para mantener el historial disponible sin conexion.
  ...
  - Se elimina la agrupacion temporal de ejercicios en bloques, superseries o circuitos porque solo modificaba la presentacion y no intervenia en la captura, el orden ni el historial. La sesion mantiene el registro libre de cualquier serie y descarta el estado, los tipos, las propiedades y las traducciones que sostenian esa funcion redundante; agrupaciones antiguas guardadas localmente se ignoran sin afectar el progreso capturado.
  ...
  - En tema claro, la barra Liquid Glass conserva un unico contorno: se retiro la capa decorativa interna y la sombra externa que se percibian como una segunda linea por debajo de la capsula.
  ...
  - La finalizacion de una sesion detecta progresion de carga dentro de esa misma sesion: cuando todas las series planificadas de una instancia externa en kilogramos se registran con al menos una repeticion y un mismo peso superior, las series de la rutina adoptan la nueva carga aunque no se alcance el objetivo completo de repeticiones. Historial y progresion se guardan en la misma transaccion, las sesiones canceladas no modifican la rutina, los reintentos son idempotentes y la cola offline conserva el peso actualizado en la copia local hasta sincronizar.
  - Historial incorpora un selector fijo entre sesiones y Progreso, sin ubicar los informes debajo de la lista cronologica ni sumar un sexto acceso al footer. Progreso admite 30, 60 y 90 dias, 6 meses, 1 ano y rangos personalizados; agrupa automaticamente por dia, semana o mes y presenta sesiones, dias entrenados, volumen externo, tiempo, series, repeticiones y ejercicios unicos. Los graficos tactiles de frecuencia, volumen, duracion y evolucion por ejercicio pueden ampliarse, y el Dashboard dispone de un acceso directo.
  - Los informes se calculan mediante RPC autenticadas y `security invoker`, respetando RLS y devolviendo series agregadas en lugar de descargar todo el historial al telefono. Se incorporan indices compuestos para sesiones y registros, la vista de Progreso se carga como bloque diferido y el volumen mantiene separados los kilogramos externos de los ejercicios por tiempo y peso corporal.
  - Progreso se organiza en Resumen, Ejercicios, Rutinas y Records. Incorpora mapa de actividad, comparacion por rutina y dia, records del periodo y estimacion Epley de 1RM para series externas de una a doce repeticiones. Los objetivos de cada serie y los nombres historicos se conservan mediante snapshots; el cumplimiento planificado frente a realizado comienza con las sesiones nuevas y no reconstruye datos antiguos con la rutina actual.
  - La vista ampliada del informe reordena las metricas y graficos para pantallas grandes y formato A4 horizontal. Permite imprimir o guardar como PDF mediante el sistema y descargar un CSV con resumen, evolucion, rutinas, dias, records y 1RM estimados, sin sumar una dependencia de generacion de documentos al bundle.
  - Resumen incorpora consistencia semanal y distribucion por grupo muscular. La consistencia compara semanas activas con las semanas del periodo y calcula la mejor racha consecutiva; la distribucion usa series completadas, no solo kilogramos, para incluir peso corporal y trabajo por tiempo. Cada registro conserva un snapshot del grupo principal del catalogo y el informe ampliado y el CSV incluyen estas nuevas metricas.

- Verificacion:
  - La linea base queda en 122 pruebas aprobadas, TypeScript sin errores y build correcto. El build PWA genera el service worker y precarga 67 recursos (aproximadamente 3,90 MiB, incluidas las fuentes locales); Progreso queda aislado en un bloque diferido de aproximadamente 37 kB y el informe exportable se carga solo al solicitarlo en otro bloque de aproximadamente 13 kB, sin reintroducir advertencias por tamano de chunks.

Correccion visual del footer en tema claro: una comparacion local en Edge reprodujo el segundo arco gris dentro de la burbuja seleccionada. Se desactiva su pseudo-elemento `::after`, que agregaba un borde interno y una sombra inferior. Los ajustes previos sobre la capsula exterior no corregian esta capa del indicador activo.
