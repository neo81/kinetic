...
- Idioma y formatos:
  ...
  - La navegacion inferior adopta una superficie compacta Liquid Glass comun a navegador y PWA. Reune cinco accesos mediante iconos de inicio, rutinas, busqueda, historial y el avatar del perfil, todos con etiquetas accesibles. La lente se desplaza al tocar o arrastrar, encaja en el destino mas cercano y aumenta temporalmente junto con el icono bajo su centro.
  ...
  - La PWA incorpora un service worker de produccion con estrategia de actualizacion controlada: precarga la estructura, vistas diferidas y recursos esenciales, elimina caches obsoletas y revisa versiones periodicamente. La interfaz informa cuando la app queda disponible sin conexion y solicita confirmacion antes de aplicar una version nueva. Si existe una sesion activa, conserva la actualizacion en espera y vuelve a ofrecerla cuando termina para evitar interrupciones.
